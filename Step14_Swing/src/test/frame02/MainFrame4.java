...
public class MainFrame4 extends JFrame implements ActionListener{
	...
	public MainFrame4(String title) {
		...
		add(deleteBtn);
		// 버튼에 리스너 등록하기
		sendBtn.addActionListener(this); // this.listener 생략, 필드 참조할때는 this
		updateBtn.addActionListener(this);
		deleteBtn.addActionListener(this);
		// 버튼에 ActionCommand를 원하는대로 지정 할 수 있다
	...
	// MainFrame4가 implements ActionListener 했으므로 감제 구현된 메소드
	@Override
	public void actionPerformed(ActionEvent e) {
		// 이벤트가 일어난 UI(Button)의 ActionCommand 값을 읽어온다 
		String command=e.getActionCommand();
		// 문자열(String)의 내용을 비교 할 때는 == 를 사용하지 말고 (참조값이 다를 수도 있기 때문)
		// .equals() 메소드를 이용해야 한다
