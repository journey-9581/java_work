...
public class MyFrame extends JFrame{
	// 생성자
	public MyFrame() {
		setLayout(null); // = this.setLayout(null);
		JButton btn=new JButton("클릭");
		btn.setBounds(50, 50, 100, 40);
		add(btn); // = this.add(btn);
		
		ActionListener listener=new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// 바깥에 감싸고 있는 클래스의 참조값은 바깥클래스명.this
	...
	// class 안에는 inner class, field, method, constructor 가능
	// 필드에는 간단한 선언만 가능(값 대입, new 정도)
} // 클래스를 닫는 중괄호
