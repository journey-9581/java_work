...
public class ServerMain {
	...
	static List<ServerThread> threadList; //static 안에서는 static만 쓸 수 있음, main 메소드도 static (static을 정의해주는 이유)
	...
	static class ServerThread extends Thread{
		...
		public ServerThread(Socket socket) {
			//생성자의 인자로 전달 받은 Socket 객체의 참조값을 필드에 저장하기
			this.socket=socket;
		}
		
		//인자로 전달되는 문자열을 Socket 객체를 통해서 출력하는 메소드, 메소드를 만들어 놨으니깐 t.broadcast("hi")가능
		public void broadcast(String msg) throws IOException {
			//인자로 전달된 문자열을 필드에 저장된 BufferedWriter 객체의 참조값을 이용해서 클라이언트에 출력하기
			bw.write(msg);
			bw.newLine();//개행기호
			bw.flush();//방출
		}
		...
		public void run() {
			try {
				...
				//반복문 돌면서 클라이언트가 전송하는 문자열이 있는지 읽어와본다
				while(true) {
					//문자열 한줄이 전송될때까지 블록킹 되는 메소드
